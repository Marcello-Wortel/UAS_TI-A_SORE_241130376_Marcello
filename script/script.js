document.querySelectorAll('.kartu-produk').forEach(card =>{
    ...
    const kurang = card.querySelector('.kurang');
    const tambah = card.querySelector('.tanbah');
    ...
    harga.innerHTML = `$${(nominalHarga) * input.value}`
    });
});
...
const orderForm = document.querySelector('.order-section from');
...
orderForm.addEventListener('submit', (e) => {
    ...
    document.querySelectorAll('.kartu-produk').forEach(card => {
        ...
        const qty = card.querySelector('quantity inbput').value;
